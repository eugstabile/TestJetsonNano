...
void Camera::StartStream()
{
    ...
    if (reqbuffers.count) {
        for (uint32_t i = 0; i < reqbuffers.count; i++) {
            ...
            if(NvBufferCreateEx(&fd_local, &cParams) < 0)
            {
				std::cerr << "Failed to create NvBuffer number" << i << std::endl;
            }
            dmabuffers_fd[i] = fd_local;
        }
    }
    else {
        for (uint32_t i = 0; i < 10; i++) {
            if (dmabuffers_fd[i]) {
                NvBufferDestroy(dmabuffers_fd[i]);
                dmabuffers_fd[i] = 0;
            }
        }
    }
...
{  
    ...
    tempDmabufIndex = v4l2_buf.m.planes[0].m.fd;
	NvBufferParams parm;
	...
	if (NvBufferGetParams(dmabuf_fd, &parm) != 0) {
		throw std::runtime_error("Get NvBufferParams failed");
	}
	
	if (NvBufferMemMap(dmabuf_fd, 0, NvBufferMem_Read_Write, &dataY) == 0) {
		NvBufferMemSyncForCpu(dmabuf_fd, 0, &dataY);
		image.strideY = parm.pitch[0];
		image.planeY = (char*)dataY;
	}	
		
	if (NvBufferMemMap(dmabuf_fd, 1, NvBufferMem_Read_Write, &dataUV) == 0) {    
		NvBufferMemSyncForCpu(dmabuf_fd, 1, &dataUV);
		image.strideUV = parm.pitch[1];
...
{
	NvBufferMemUnMap(tempDmabufIndex, 0, &dataY); 
	NvBufferMemUnMap(tempDmabufIndex, 1, &dataUV); 
